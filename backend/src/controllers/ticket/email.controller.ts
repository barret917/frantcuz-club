...
import { formatDate, formatTime } from '../dateFormatter.controller';
        ...
        <div style="margin-left: 15px;">
            ...
            <div style="display: flex; justify-content: space-between;">
                <div style="flex: 1;">
                    ...
                    <p style="margin: 5px 0;"><strong>Стоимость:</strong> ${eventData.price} руб.</p>
                </div>
                <div style="flex: 0 0 150px; text-align: center;">
                    ...
                    ` : '<p style="color: #777;">QR-код не сгенерирован</p>'}
                </div>
            </div>
        </div>
    </div>
...
export const sendTicketsToCustomer = async (userEmail: string, order: Order, userTickets: Ticket[]): Promise<boolean> => {
    try {
        ...
        // Подготовка вложений
        const attachments = userTickets
            .filter(ticket => ticket.qr_code && ticket.qr_code.includes(','))
            .map(ticket => ({
                filename: `Билет_${ticket.ticket_number}.png`,
                content: ticket.qr_code!.split(',')[1],
