...
async function seedMenu() {
    try {
        ...
        // Создаем типы меню
        const mainMenu = await prisma_1.prisma.menuType.upsert({
            where: { slug: 'main' },
            update: {},
            create: {
                name: 'Основное',
        ...
        const barMenu = await prisma_1.prisma.menuType.upsert({
            where: { slug: 'bar' },
            update: {},
            create: {
                name: 'Барная карта',
        ...
        const banquetMenu = await prisma_1.prisma.menuType.upsert({
            where: { slug: 'banquet' },
            update: {},
            create: {
                name: 'Банкетное меню',
        ...
        for (const category of mainCategories) {
            await prisma_1.prisma.menuCategory.upsert({
        ...
        const coldAppetizers = await prisma_1.prisma.menuCategory.findUnique({ where: { slug: 'cold-appetizers' } });
        const salads = await prisma_1.prisma.menuCategory.findUnique({ where: { slug: 'salads' } });
        const cocktails = await prisma_1.prisma.menuCategory.findUnique({ where: { slug: 'cocktails' } });
        const beer = await prisma_1.prisma.menuCategory.findUnique({ where: { slug: 'beer' } });
        const banquetSets = await prisma_1.prisma.menuCategory.findUnique({ where: { slug: 'banquet-sets' } });
        // Создаем примеры блюд
        if (coldAppetizers) {
            await prisma_1.prisma.menuItem.upsert({
                where: { id: 1 },
                update: {},
                create: {
                    ...
                    price: 650.00,
                    currency: '₽',
                    imageUrl: 'data:image/svg+xml;base64,',
